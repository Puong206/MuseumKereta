...
namespace MuseumApp
{
    public partial class Kelola_Perawatan : Page
    {
        ...
        private void BtnImportExcelPerawatan_Click(object sender, RoutedEventArgs e)
        {
            OpenFileDialog openFileDialog = new OpenFileDialog
            {
                Filter = "Excel Files (*.xlsx)|*.xlsx",
                Title = "Pilih File Excel untuk Impor Data Perawatan"
            };
        ...
        private void PreviewData(string filePath)
        {
            ...
            try
            {
                using (FileStream fs = new FileStream(filePath, FileMode.Open, FileAccess.Read)) 
                {
                    ...
                    string[] requiredColumns = { "BarangID", "TanggalPerawatan", "JenisPerawatan", "NIPP" };
                    ...
                    if (!dataTable.Columns.Contains("Catatan"))
                    {
                    ...
                    for (int i = 1; i <= sheet.LastRowNum; i++)
                    {
                        ...
                        for (int j = 0; j < headerRow.LastCellNum; j++)
                        {
                            if (j < dataTable.Columns.Count)
                            {
                                ...
                                if (cell != null)
                                {
                                    // Penanganan khusus untuk sel tanggal
                                    if (dtExcel.Columns[j].ColumnName == "TanggalPerawatan" && cell.CellType == CellType.Numeric && DateUtil.IsCellDateFormatted(cell))
                                    {
                                        cellValue = cell.DateCellValue.ToString("yyyy-MM-dd"); // Format standar yang mudah di-parse
                                    }
                                    else
                                    {
                ...
                {
                    // Panggil PreviewWindow yang fleksibel dengan tipe data Perawatan
                    PreviewWindow preview = new PreviewWindow(dataTable, TipeDataImport.Perawatan, this.connectionString);
